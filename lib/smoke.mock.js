...
Smoke.reset = function(){
  if (Smoke.mocks) {
    for(var i=0; i<Smoke.mocks.length; i++) Smoke.mocks[i].reset();
  }
	Smoke.mocks = [];
	Smoke.passCount = 0;
	Smoke.failCount = 0;
...
Smoke.Mock = function(originalObj) {
	...
	obj.should_receive = function(attr){
		...
		if (this._previousFunctions[attr] === undefined) {
		  this._previousFunctions[attr] = previousFunction;
		}
		var mock = this;
	...
	obj.reset = function(){
	  for(var attr in this._previousFunctions) {
	    this[attr] = this._previousFunctions[attr];
    }
	};
...
Smoke.Mock.Expectation.prototype = {
	...
	run: function(args){
      ...
      return this.returnValue;
		...
		else if(typeof(this.callerArgs) == 'undefined' || this.compareArrays(args, this.callerArgs)) {
			this.callCount+=1;
			return true;
		};
		return false
